 ...
 * 实现 {@link DataSource} 接口，池化的 {@link DataSource} 实现类
 *
...
public class PooledDataSource implements DataSource {
  ...
  /**
   * 这是一个关于坏连接容忍度的底层设置，作用于每一个尝试从缓存池获取连接的线程。如果这个线程获取得到的是一个坏的连接，那么这个数据源允许这个线程尝试重新获取一个新的连接，但是这个重新尝试的次数不应该超过 poolMaximumIdleConnections 与 poolMaximumLocalBadConnectionTolerance 之和。
   */
   ...
   * 将使用完的连接，添加回连接池中
   *
   * @param conn PooledConnection 对象
   * @throws SQLException
   */
    ...
    synchronized (state) {
      ...
      if (conn.isValid()) {
        ...
        if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
          ...
          // 创建 PooledConnection 对象，并添加到空闲的连接中
  ...
  private PooledConnection popConnection(String username, String password) throws SQLException {
    ...
    int localBadConnectionCount = 0; // 记录当前方法，获取到怀连接的次数
    ...
    while (conn == null) {
      synchronized (state) {
        ...
        if (conn != null) {
          ...
          if (conn.isValid()) {
            // 如果非自动提交的，需要进行回滚。即将原有执行中的事务，全部回滚
            // 这里又执行一次，有点奇怪。目前猜测，是不是担心上一次使用方忘记提交或回滚事务
            if (!conn.getRealConnection().getAutoCommit()) {
              conn.getRealConnection().rollback();
            }
            // 设置获取连接的属性
